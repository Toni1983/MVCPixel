...
namespace SummerCampECommerce2019.Models
{
    public class ImageUpload
    {
        ...
        internal Tuple<string, string> ImageResize(HttpPostedFileBase FileUpload1, int genislik, int yukseklik, int buyukGenislik, int buyukYukseklik)
        {
            ...
            UploadedFileName = HttpContext.Current.Server.MapPath("~/images/Upload/" + DateTime.Now.ToShortDateString().Trim().Replace(':', '_').Replace('.', '_')/*.Replace('/','_')*/ + DateTime.Now.ToShortTimeString().Trim().Replace(':', '_').Replace('.', '_').Replace('/', '_') + dosyaAdi);
